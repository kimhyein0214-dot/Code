...
const CODE_SYSTEMS = [
  {
    value: 'selfpia_sku',
    label: 'selfpia_sku (Selfpia SKU code)',
    description: 'canonical SKU와 1:1로 연결되는 기본 코드입니다. 예: LOCAL_TEST_PM_1258-1',
    example: 'LOCAL_TEST_PM_1258-1'
  },
  {
    value: 'selfpia_product',
    label: 'selfpia_product (상품 단위)',
    description: '상품 단위 코드입니다. 같은 상품 아래 여러 옵션 SKU가 묶일 수 있습니다.',
    example: 'LOCAL_TEST_PM_1258'
  },
  {
    value: 'own_sku',
    label: 'own_sku (자체 옵션 코드)',
    description: 'n:m 가능성이 있어 같은 own_sku가 여러 SKU에 연결되면 복수 후보로 반환합니다.',
    example: 'LOCAL_TEST_PM_OWN_AMBIG'
  },
  {
    value: 'smartstore_option_no',
    label: 'smartstore_option_no (스마트스토어 옵션 번호)',
    description: '스마트스토어 채널 옵션 번호입니다. local seed에는 대표 샘플이 포함되어 있습니다.',
  ...
  return (
    <section className="page">
      <div className="page-header">
        <div>
          <h1>Alias 검색</h1>
          <p>code system과 code value로 연결된 SKU 후보를 조회합니다.</p>
        </div>
      </div>

      <section className="section-card" aria-label="Alias 검색 조건">
      ...
      {ambiguous && !error && (
        <div className="notice warn">
          ...
          own_sku 같은 n:m alias에서 자주 발생하며 자동 확정은 하지 않습니다.
        </div>
      )}
            ...
            <div className="alias-result-main">
              ...
              <ProductMetaChips
                items={[
                  ...
                  { key: 'matched_code_value', label: matchedCodeLabel(row.matched_code_system), value: row.matched_code_value }
                ]}
              />
            </div>
            <div className="alias-result-actions">
        ...
        {empty && (
          <div className="empty-state">
            ...
            <ul className="empty-hints">
              ...
              <li>운영 코드를 조회하려면 local DB에 master export 적재가 필요합니다.</li>
            </ul>
          </div>
        )}
      </div>
