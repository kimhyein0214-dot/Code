-- =============================================================================
-- export_makeshop_review_samples.sql
--
-- Read-only export of review_required samples by reason.
-- Output files are written inside the Docker container under /tmp.
--
-- Output:
--   /tmp/makeshop_review_null_key_sample.csv
--   /tmp/makeshop_review_pattern_unmatched_sample.csv
--   /tmp/makeshop_review_own_sku_not_in_alias_full.csv
--   /tmp/makeshop_review_own_sku_ambiguous_sample.csv
--
...
\echo
\echo ===== [review reason summary before export] =====
SELECT
  review_reason,
  COUNT(*) AS rows
FROM mk_classified
...
ORDER BY rows DESC, review_reason;

\copy (SELECT 'makeshop'::text AS channel_code, product_uid AS seller_product_code_raw, channel_sku_code, sto_id AS sto_id_raw, sto_code, opt_value, opt_values, own_sku_candidate AS own_sku_code, extraction_method, match_count, candidate_sku_ids, product_name, barcode, review_reason FROM mk_classified WHERE review_reason = 'null_key' ORDER BY product_uid, sto_id LIMIT 100) TO '/tmp/makeshop_review_null_key_sample.csv' WITH (FORMAT CSV, HEADER true, ENCODING 'UTF8')

\copy (SELECT 'makeshop'::text AS channel_code, product_uid AS seller_product_code_raw, channel_sku_code, sto_id AS sto_id_raw, sto_code, opt_value, opt_values, own_sku_candidate AS own_sku_code, extraction_method, match_count, candidate_sku_ids, product_name, barcode, review_reason FROM mk_classified WHERE review_reason = 'pattern_unmatched' ORDER BY product_uid, sto_id LIMIT 100) TO '/tmp/makeshop_review_pattern_unmatched_sample.csv' WITH (FORMAT CSV, HEADER true, ENCODING 'UTF8')

\copy (SELECT 'makeshop'::text AS channel_code, product_uid AS seller_product_code_raw, channel_sku_code, sto_id AS sto_id_raw, sto_code, opt_value, opt_values, own_sku_candidate AS own_sku_code, extraction_method, match_count, candidate_sku_ids, product_name, barcode, review_reason FROM mk_classified WHERE review_reason = 'own_sku_not_in_alias' ORDER BY own_sku_candidate, product_uid, sto_id) TO '/tmp/makeshop_review_own_sku_not_in_alias_full.csv' WITH (FORMAT CSV, HEADER true, ENCODING 'UTF8')

\copy (SELECT 'makeshop'::text AS channel_code, product_uid AS seller_product_code_raw, channel_sku_code, sto_id AS sto_id_raw, sto_code, opt_value, opt_values, own_sku_candidate AS own_sku_code, extraction_method, match_count, candidate_sku_ids, product_name, barcode, review_reason FROM mk_classified WHERE review_reason = 'own_sku_ambiguous' ORDER BY match_count DESC, own_sku_candidate, product_uid, sto_id LIMIT 200) TO '/tmp/makeshop_review_own_sku_ambiguous_sample.csv' WITH (FORMAT CSV, HEADER true, ENCODING 'UTF8')

\echo
\echo Exported review CSV files under /tmp:
\echo /tmp/makeshop_review_null_key_sample.csv
\echo /tmp/makeshop_review_pattern_unmatched_sample.csv
\echo /tmp/makeshop_review_own_sku_not_in_alias_full.csv
\echo /tmp/makeshop_review_own_sku_ambiguous_sample.csv

ROLLBACK;

\echo
\echo export_makeshop_review_samples.sql complete. ROLLBACK applied. No persistent DB changes.
