/*
  MakeShop unique evidence sample/detail review.

  Purpose:
  - Rebuild the 255 clean planned subset from the unique-evidence dryrun.
  - Output review summary plus limited detail samples before any local apply SQL.
  ...
  - No apply SQL.
  - SELECT-only when run inside a read-only transaction.
  - No file output.
  - No import/export.
  - No stage relation.
...
SELECT
  ...
  'Run inside a read-only transaction for local sample review; this SQL does not write.'::text AS note;
...
summary_rows AS (
  ...
  UNION ALL SELECT 'clean_planned_subset_count', COUNT(*)::bigint, 'clean review subset after excluding existing/manual, duplicate code pairs, selfpia splits, and semantic warnings; this SQL does not write.' FROM clean_dryrun_plan
  ...
  UNION ALL SELECT 'duplicate_makeshop_code_count', COUNT(*)::bigint, 'must be zero inside clean planned rows.' FROM clean_dryrun_plan WHERE sku_count_for_dryrun_code > 1
  UNION ALL SELECT 'duplicate_selfpia_to_makeshop_count', COUNT(*)::bigint, 'must be zero inside clean planned rows.' FROM clean_dryrun_plan WHERE product_count_for_selfpia_sku > 1
  UNION ALL SELECT 'semantic_warning_count', COUNT(*)::bigint, 'must be zero inside clean planned rows.' FROM clean_dryrun_plan WHERE semantic_or_risk_keyword_warning
  UNION ALL SELECT 'clean_crystal_keyword_count', COUNT(*)::bigint, 'clean subset rows containing crystal text; broad review signal, not automatic exclusion.' FROM clean_dryrun_plan WHERE lower(COALESCE(option_name, '')) LIKE '%크리스탈%'
  UNION ALL SELECT 'clean_crystal_ab_keyword_count', COUNT(*)::bigint, 'clean subset rows containing crystal AB text; should be reviewed as distinct from crystal.' FROM clean_dryrun_plan WHERE lower(COALESCE(option_name, '')) LIKE '%크리스탈ab%' OR lower(COALESCE(option_name, '')) LIKE '%크리ab%'
  UNION ALL SELECT 'clean_ab_keyword_count', COUNT(*)::bigint, 'clean subset rows containing AB token/text.' FROM clean_dryrun_plan WHERE lower(COALESCE(option_name, '')) LIKE '%ab%'
  UNION ALL SELECT 'clean_white_gold_keyword_count', COUNT(*)::bigint, 'clean subset rows containing white gold text.' FROM clean_dryrun_plan WHERE lower(COALESCE(option_name, '')) LIKE '%화이트골드%'
  UNION ALL SELECT 'clean_silver_keyword_count', COUNT(*)::bigint, 'clean subset rows containing silver text; broad review signal, not automatic exclusion.' FROM clean_dryrun_plan WHERE lower(COALESCE(option_name, '')) LIKE '%실버%'
  UNION ALL SELECT 'clean_gold_keyword_count', COUNT(*)::bigint, 'clean subset rows containing gold text; broad review signal, not automatic exclusion.' FROM clean_dryrun_plan WHERE lower(COALESCE(option_name, '')) LIKE '%골드%'
  UNION ALL SELECT 'clean_rose_gold_keyword_count', COUNT(*)::bigint, 'clean subset rows containing rose gold text.' FROM clean_dryrun_plan WHERE lower(COALESCE(option_name, '')) LIKE '%로즈골드%'
  UNION ALL SELECT 'clean_pink_gold_keyword_count', COUNT(*)::bigint, 'clean subset rows containing pink gold text.' FROM clean_dryrun_plan WHERE lower(COALESCE(option_name, '')) LIKE '%핑크골드%'
  UNION ALL SELECT 'clean_set_keyword_count', COUNT(*)::bigint, 'clean subset rows containing set text.' FROM clean_dryrun_plan WHERE lower(COALESCE(option_name, '')) LIKE '%세트%'
  UNION ALL SELECT 'clean_one_plus_one_keyword_count', COUNT(*)::bigint, 'clean subset rows containing 1+1 text.' FROM clean_dryrun_plan WHERE lower(COALESCE(option_name, '')) LIKE '%1+1%'
  UNION ALL SELECT 'clean_quantity_keyword_count', COUNT(*)::bigint, 'clean subset rows containing quantity text.' FROM clean_dryrun_plan WHERE lower(COALESCE(option_name, '')) LIKE '%수량%'
  UNION ALL SELECT 'review_verdict', CASE
      WHEN (SELECT COUNT(*) FROM candidate_rows) = 1247
       ...
       AND NOT EXISTS (SELECT 1 FROM clean_dryrun_plan WHERE sku_count_for_dryrun_code > 1)
       AND NOT EXISTS (SELECT 1 FROM clean_dryrun_plan WHERE product_count_for_selfpia_sku > 1)
       AND NOT EXISTS (SELECT 1 FROM clean_dryrun_plan WHERE semantic_or_risk_keyword_warning)
      THEN 1 ELSE 0 END::bigint,
    'PASS when row_count=1; FAIL when row_count=0.'
),
review_bucket_source AS (
  SELECT
    'clean_subset_sample'::text AS sample_bucket,
    d.confidence_tier,
    d.sku_id,
    d.sku_id_text,
    d.selfpia_sku,
    ...
    'clean subset; review before local apply design' AS risk_note,
    d.export_allowed,
    d.reviewer_decision
  FROM clean_dryrun_plan AS d
  UNION ALL
  SELECT
...
SELECT
  ...
  CASE summary_type
    ...
    WHEN 'clean_planned_subset_count' THEN 8
    WHEN 'skipped_existing_confirmed_count' THEN 9
    WHEN 'skipped_existing_manual_count' THEN 10
    WHEN 'duplicate_makeshop_code_count' THEN 11
    WHEN 'duplicate_selfpia_to_makeshop_count' THEN 12
    WHEN 'semantic_warning_count' THEN 13
    WHEN 'clean_crystal_keyword_count' THEN 14
    WHEN 'clean_crystal_ab_keyword_count' THEN 15
    WHEN 'clean_ab_keyword_count' THEN 16
    WHEN 'clean_white_gold_keyword_count' THEN 17
    WHEN 'clean_silver_keyword_count' THEN 18
    WHEN 'clean_gold_keyword_count' THEN 19
    WHEN 'clean_rose_gold_keyword_count' THEN 20
    WHEN 'clean_pink_gold_keyword_count' THEN 21
    WHEN 'clean_set_keyword_count' THEN 22
    WHEN 'clean_one_plus_one_keyword_count' THEN 23
    WHEN 'clean_quantity_keyword_count' THEN 24
    WHEN 'review_verdict' THEN 25
    ELSE 99
  END AS summary_sort,
  99 AS bucket_sort,
...
SELECT
  ...
  99 AS summary_sort,
  CASE sample_bucket
    WHEN 'clean_subset_sample' THEN 1
    WHEN 'same_product_family_clean_sample' THEN 2
    WHEN 'cross_product_possible_clean_sample' THEN 3
    WHEN 'seller_product_code_group_sample' THEN 4
    WHEN 'channel_sku_code_sample' THEN 5
    WHEN 'selfpia_sku_sample' THEN 6
    WHEN 'medium_candidate_sample' THEN 7
    WHEN 'makeshop_code_present_sample' THEN 8
    WHEN 'makeshop_code_missing_sample' THEN 9
    WHEN 'own_sku_unique_code_evidence_sample' THEN 10
    WHEN 'duplicate_evidence_excluded_sample' THEN 11
    WHEN 'duplicate_code_pair_excluded_sample' THEN 12
    WHEN 'evidence_missing_excluded_sample' THEN 13
    WHEN 'risk_keyword_sample' THEN 14
    WHEN 'same_product_family_sample' THEN 15
    WHEN 'own_sku_repeat_sample' THEN 16
    WHEN 'random_sample' THEN 17
    WHEN 'risk_edge_sample' THEN 18
    ELSE 99
  END AS bucket_sort,
  ...
  risk_note,
  CASE
    WHEN sample_bucket = 'clean_subset_sample' THEN 'Review clean subset rows before any local apply SQL is written.'
  ...
  sample_rank
FROM ranked_samples
WHERE sample_rank <= CASE
  WHEN sample_bucket = 'clean_subset_sample' THEN 80
  ELSE 20
END
)
SELECT
