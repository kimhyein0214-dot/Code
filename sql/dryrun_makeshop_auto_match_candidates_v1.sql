...
SELECT 'execution_context'::text AS dryrun_item, 1::bigint AS row_count, current_database() || ' / ' || current_user AS note;
...
dryrun_summary AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE (promotable_to_high_confidence OR promotable_to_medium_confidence) AND NOT matched_confirmed AND NOT semantic_true_risk AND NOT duplicate_selfpia_sku_to_makeshop_code AND NOT channel_absent_or_inactive) AS insert_or_update_planned_count,
    COUNT(*) FILTER (WHERE (promotable_to_high_confidence OR promotable_to_medium_confidence) AND matched_confirmed) AS skipped_existing_confirmed_count,
    0::bigint AS skipped_existing_manual_count,
    COUNT(*) FILTER (WHERE matching_presence_status = 'blocked_risk' AND NOT promotable_to_high_confidence AND NOT promotable_to_medium_confidence) AS skipped_blocked_risk_count,
    COUNT(*) FILTER (WHERE matching_presence_status = 'channel_absent_or_inactive') AS skipped_channel_absent_or_inactive_count,
    ...
    COUNT(*) FILTER (WHERE (promotable_to_high_confidence OR promotable_to_medium_confidence) AND NOT matched_confirmed AND NOT semantic_true_risk AND NOT duplicate_selfpia_sku_to_makeshop_code AND NOT channel_absent_or_inactive) AS expected_after_count
  FROM promotion
)
SELECT 'candidate_total'::text AS dryrun_item, candidate_total AS row_count, 'MakeShop high + medium candidates considered by this rollback-only dryrun.'::text AS note FROM dryrun_summary
UNION ALL SELECT 'high_candidate_count', high_candidate_count, 'high confidence candidates.' FROM dryrun_summary
UNION ALL SELECT 'medium_candidate_count', medium_candidate_count, 'medium confidence candidates.' FROM dryrun_summary
UNION ALL SELECT 'insert_or_update_planned_count', insert_or_update_planned_count, 'planned later-apply rows; no row is persisted here.' FROM dryrun_summary
UNION ALL SELECT 'skipped_existing_confirmed_count', skipped_existing_confirmed_count, 'candidate rows skipped because confirmed MakeShop evidence already exists.' FROM dryrun_summary
UNION ALL SELECT 'skipped_existing_manual_count', skipped_existing_manual_count, 'manual guard placeholder; no manual table is written by this dryrun.' FROM dryrun_summary
UNION ALL SELECT 'skipped_blocked_risk_count', skipped_blocked_risk_count, 'blocked risk rows excluded from later apply.' FROM dryrun_summary
UNION ALL SELECT 'skipped_channel_absent_or_inactive_count', skipped_channel_absent_or_inactive_count, 'channel absent/inactive rows excluded from later apply.' FROM dryrun_summary
UNION ALL SELECT 'duplicate_makeshop_code_count', duplicate_makeshop_code_count, 'must be zero inside planned rows.' FROM dryrun_summary
UNION ALL SELECT 'duplicate_selfpia_to_makeshop_count', duplicate_selfpia_to_makeshop_count, 'must be zero inside planned rows.' FROM dryrun_summary
UNION ALL SELECT 'own_sku_multi_conflict_count', own_sku_multi_conflict_count, 'expected non-zero; own_sku repeats are reclassified evidence.' FROM dryrun_summary
UNION ALL SELECT 'semantic_warning_count', semantic_warning_count, 'semantic warning rows are not planned for later apply.' FROM dryrun_summary
UNION ALL SELECT 'expected_after_count', expected_after_count, 'same as planned later-apply rows for this rollback-only dryrun.' FROM dryrun_summary;

ROLLBACK;

WITH rollback_check AS (
  SELECT COUNT(*) AS rollback_after_count
  FROM product_code.code_alias AS ca
  WHERE ca.source_project_ref = 'makeshop_auto_match_dryrun_v1'
)
SELECT 'rollback_after_count'::text AS dryrun_item, rollback_after_count AS row_count, 'must be zero after rollback-only dryrun.'::text AS note FROM rollback_check
UNION ALL SELECT 'rollback_verdict', CASE WHEN rollback_after_count = 0 THEN 1 ELSE 0 END, CASE WHEN rollback_after_count = 0 THEN 'PASS' ELSE 'FAIL' END FROM rollback_check;
