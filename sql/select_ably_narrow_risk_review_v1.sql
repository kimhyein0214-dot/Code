/*
  Select Ably narrow risk review v1.
  ...
  Purpose:
  - Review only narrow risk patterns inside the Ably final planned 3,024
...
SELECT
  ...
  CASE
    ...
    ELSE 'STOP'
  END AS guard_result,
  'Ably narrow risk review; SELECT-only'::text AS note;
...
classified AS MATERIALIZED (
  SELECT
    ...
    (rr.review_text ~ '(실버|골드|핑크골드|로즈골드|옐로우골드|크리스탈)') AS has_broad_color,
    (
...
source_conflict_sample AS MATERIALIZED (
  SELECT DISTINCT ON (cu.source_system, cu.channel_product_code, cu.sku_id)
    cu.evidence_id,
    cu.source_system,
    cu.source_row_no,
    cu.source_option_line_no,
    ...
    cu.product_name,
    cu.option_value
  FROM source_conflict_pair AS sc
  JOIN clean_unique AS cu
    ON cu.source_system = sc.source_system
   AND cu.channel_product_code = sc.channel_product_code
   AND cu.sku_id = sc.sku_id
  ORDER BY cu.source_system, cu.channel_product_code, cu.sku_id, cu.evidence_id
  LIMIT 50
)
SELECT
  ...
  jsonb_build_object(
    ...
    'safe_broad_color_only_count', (SELECT COUNT(*) FROM classified WHERE has_broad_color AND NOT has_narrow_risk),
    ...
    'apply_dryrun_ready_verdict',
      CASE
        WHEN (SELECT COUNT(*) FROM duplicate_after_exclusion) = 0
...
UNION ALL
SELECT
  'source_conflict_reference_sample',
  row_number() OVER (ORDER BY source_system, channel_product_code, sku_id)::integer,
  to_jsonb(source_conflict_sample)
FROM source_conflict_sample
