/*
  MakeShop auto-match postcheck draft.

  Run this only after a separately approved local apply.
  This file is SELECT-only and expects future applied rows to carry:
  - source_project_ref = 'makeshop_auto_match_dryrun_v1'
  - MakeShop code systems or sku_channel_mapping channel_code = 'makeshop'
*/
...
WITH applied_alias AS (
  SELECT
    ...
    COUNT(*) FILTER (
      WHERE ca.code_system = 'makeshop_product_code'
    ) AS applied_product_code_rows,
    COUNT(*) FILTER (
      WHERE ca.code_system = 'makeshop_option_code'
    ) AS applied_option_code_rows,
    COUNT(DISTINCT ca.code_value) FILTER (
      WHERE ca.code_system = 'makeshop_product_code'
    ) AS applied_product_code_distinct_count,
    COUNT(DISTINCT ca.code_value) FILTER (
      WHERE ca.code_system = 'makeshop_option_code'
    ) AS applied_option_code_distinct_count
  FROM product_code.code_alias AS ca
  WHERE ca.target_type = 'SKU'
    AND ca.source_project_ref = 'makeshop_auto_match_dryrun_v1'
...
applied_mapping AS (
  ...
  WHERE lower(scm.channel_code) = 'makeshop'
    AND COALESCE(scm.raw_payload::text, '') LIKE '%makeshop_auto_match_dryrun_v1%'
  GROUP BY scm.sku_id
),
...
makeshop_code_duplicates AS (
  SELECT
    COALESCE(ca_product.code_value, scm.seller_product_code) AS makeshop_code,
    COALESCE(ca_option.code_value, scm.channel_sku_code) AS makeshop_option,
...
selfpia_makeshop_splits AS (
  SELECT
    v.selfpia_sku_code,
    COUNT(DISTINCT COALESCE(ca_product.code_value, scm.seller_product_code)) AS makeshop_code_count
  FROM applied_rows AS ar
...
SELECT
  'makeshop_auto_matched_count'::text AS postcheck_item,
  (SELECT COUNT(*) FROM applied_rows)::bigint AS row_count,
  'rows tagged by future approved apply marker.'::text AS note
UNION ALL
SELECT
  'duplicate_makeshop_code_residual',
  COUNT(*)::bigint,
  'must be zero.'
FROM makeshop_code_duplicates
WHERE sku_count > 1
UNION ALL
SELECT
  'duplicate_selfpia_to_makeshop_residual',
  COUNT(*)::bigint,
  'must be zero.'
FROM selfpia_makeshop_splits
WHERE makeshop_code_count > 1
UNION ALL
SELECT
...
SELECT
  'final_verdict',
  CASE
    WHEN NOT EXISTS (SELECT 1 FROM makeshop_code_duplicates WHERE sku_count > 1)
     AND NOT EXISTS (SELECT 1 FROM selfpia_makeshop_splits WHERE makeshop_code_count > 1)
