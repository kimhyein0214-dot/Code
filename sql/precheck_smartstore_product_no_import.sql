...
-- 목적       : smartstore productNo import 전에 local 상태를 stamp.
--             apply 후 postcheck 와 비교용.
-- =============================================================================

-- 0) staging 테이블 존재 여부
select
  to_regclass('product_code.smartstore_product_no_stage') as stage_relation;
...
-- 2) smartstore productNo 관련 alias 잔존 여부 (기대값 0)
...
-- 3) 핵심 카운트 stamp — apply 후 postcheck 와 1:1 비교용
select
...
-- 5) 예시 SKU 1000-3 의 현재 alias (smartstore_product_no 0건이어야 함)
