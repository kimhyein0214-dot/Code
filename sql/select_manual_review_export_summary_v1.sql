/*
  상세 row 출력 금지.
  수동검수 export SELECT 결과 요약용.
  SELECT-only.
  local DB read-only 확인용.
  DB 쓰기 없음.
  파일 출력 없음.
*/
...
summary_rows AS (
  SELECT
    10 AS summary_order,
    'total_review_rows'::text AS summary_area,
    NULL::text AS source_channel,
    NULL::text AS review_reason,
    NULL::boolean AS export_allowed,
    NULL::text AS reviewer_decision,
    NULL::boolean AS has_confirmed_product_no,
    NULL::boolean AS has_confirmed_option_no,
    NULL::boolean AS has_candidate_product_no,
    NULL::boolean AS has_candidate_option_no,
  ...
  SELECT
    20 AS summary_order,
    'source_channel_count'::text AS summary_area,
    source_channel,
    NULL::text AS review_reason,
    NULL::boolean AS export_allowed,
    NULL::text AS reviewer_decision,
  ...
  SELECT
    30 AS summary_order,
    'review_reason_count'::text AS summary_area,
    NULL::text AS source_channel,
    review_reason,
    NULL::boolean AS export_allowed,
    NULL::text AS reviewer_decision,
  ...
  SELECT
    40 AS summary_order,
    'source_channel_review_reason_count'::text AS summary_area,
    source_channel,
    review_reason,
    NULL::boolean AS export_allowed,
    NULL::text AS reviewer_decision,
    ...
    COUNT(*)::bigint AS row_count
  FROM review_targets
  GROUP BY source_channel, review_reason

  UNION ALL

  SELECT
    50 AS summary_order,
    'export_allowed_count'::text AS summary_area,
    NULL::text AS source_channel,
    NULL::text AS review_reason,
    export_allowed,
    NULL::text AS reviewer_decision,
    ...
    COUNT(*)::bigint AS row_count
  FROM review_targets
  GROUP BY export_allowed

  UNION ALL

  SELECT
    60 AS summary_order,
    'reviewer_decision_count'::text AS summary_area,
    NULL::text AS source_channel,
    NULL::text AS review_reason,
    NULL::boolean AS export_allowed,
    reviewer_decision,
  ...
  SELECT
    70 AS summary_order,
    'smartstore_confirmed_candidate_distribution'::text AS summary_area,
    source_channel,
    NULL::text AS review_reason,
    NULL::boolean AS export_allowed,
    NULL::text AS reviewer_decision,
    (confirmed_product_no IS NOT NULL)::boolean AS has_confirmed_product_no,
    (confirmed_option_no IS NOT NULL)::boolean AS has_confirmed_option_no,
    (candidate_product_no IS NOT NULL)::boolean AS has_candidate_product_no,
    (candidate_option_no IS NOT NULL)::boolean AS has_candidate_option_no,
    COUNT(*)::bigint AS row_count
  FROM review_targets
  WHERE source_channel = 'smartstore'
  GROUP BY
    source_channel,
    (confirmed_product_no IS NOT NULL),
    (confirmed_option_no IS NOT NULL),
    (candidate_product_no IS NOT NULL),
    (candidate_option_no IS NOT NULL)
)
...
SELECT
  ...
  export_allowed,
  reviewer_decision,
  has_confirmed_product_no,
  has_confirmed_option_no,
  has_candidate_product_no,
  has_candidate_option_no,
  row_count
FROM summary_rows
ORDER BY
  summary_order,
  source_channel NULLS FIRST,
  review_reason NULLS FIRST,
  export_allowed NULLS FIRST,
  reviewer_decision NULLS FIRST,
  has_confirmed_product_no NULLS FIRST,
  has_confirmed_option_no NULLS FIRST,
  has_candidate_product_no NULLS FIRST,
  has_candidate_option_no NULLS FIRST;
