...
candidate_rows AS (
  SELECT
    CASE
      WHEN promotable_to_high_confidence THEN 'auto_match_high_confidence'
      WHEN promotable_to_medium_confidence THEN 'auto_match_medium_confidence'
      ELSE 'not_candidate'
    ...
    makeshop_code_key AS makeshop_code_candidate,
    makeshop_option_key AS makeshop_option_candidate,
    CASE
      WHEN promotable_to_high_confidence THEN 'unique MakeShop code + same product family evidence'
      WHEN promotable_to_medium_confidence THEN 'own_sku repeat reclassified with MakeShop or normalization evidence'
      ELSE 'excluded'
    END AS match_reason,
    CASE
      WHEN semantic_true_risk THEN 'semantic risk remains'
      WHEN duplicate_selfpia_sku_to_makeshop_code THEN 'selfpia SKU splits to multiple MakeShop codes'
      WHEN channel_absent_or_inactive THEN 'channel absent or inactive'
      WHEN true_conflict_or_residual THEN 'remain blocked risk'
      ELSE 'dryrun candidate'
    END AS risk_note,
    false::boolean AS export_allowed,
    'pending'::text AS reviewer_decision
  FROM promotion
  WHERE promotable_to_high_confidence
     OR promotable_to_medium_confidence
),
summary_rows AS (
  SELECT 'candidate_total'::text AS result_type, COUNT(*)::bigint AS row_count, 'high + medium MakeShop candidates.'::text AS note FROM candidate_rows
  UNION ALL SELECT 'high_candidate_count', COUNT(*)::bigint, 'high confidence candidates.' FROM candidate_rows WHERE confidence_tier = 'auto_match_high_confidence'
...
sample_rows AS (
  SELECT
    'sample'::text AS result_type,
    ROW_NUMBER() OVER (PARTITION BY confidence_tier ORDER BY selfpia_sku, sku_id)::bigint AS row_count,
    confidence_tier || ' | ' || selfpia_sku || ' | ' || COALESCE(makeshop_code_candidate, '') || ' | ' || COALESCE(makeshop_option_candidate, '') || ' | ' || match_reason AS note
  FROM candidate_rows
)
SELECT result_type, row_count, note
FROM summary_rows
UNION ALL
SELECT result_type, row_count, note
FROM sample_rows
WHERE row_count <= 20
ORDER BY result_type, row_count;
