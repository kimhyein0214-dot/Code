/*
  Smartstore auto-match dryrun workflow.

  This file is intentionally rollback-only and read-only:
  - It plans the rows that would be promoted later.
  - It does not persist any row.
...
SELECT
  'execution_context'::text AS dryrun_item,
...
dryrun_summary AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE is_planned_for_later_apply) AS insert_or_update_planned_count,
    ...
    COUNT(*) FILTER (WHERE is_planned_for_later_apply) AS expected_after_count
  FROM planned_candidates
)

SELECT 'candidate_total'::text AS dryrun_item, candidate_total AS row_count, 'high + medium candidates considered by this dryrun.'::text AS note FROM dryrun_summary
UNION ALL SELECT 'high_candidate_count', high_candidate_count, 'high confidence candidates before skip guards.' FROM dryrun_summary
UNION ALL SELECT 'medium_candidate_count', medium_candidate_count, 'medium confidence candidates before skip guards.' FROM dryrun_summary
UNION ALL SELECT 'insert_or_update_planned_count', insert_or_update_planned_count, 'planned later-apply rows after confirmed/manual/risk guards; no row is persisted here.' FROM dryrun_summary
UNION ALL SELECT 'skipped_existing_confirmed_count', skipped_existing_confirmed_count, 'candidate rows skipped because confirmed Smartstore evidence already exists.' FROM dryrun_summary
UNION ALL SELECT 'skipped_existing_manual_count', skipped_existing_manual_count, 'candidate rows skipped because manual/reviewer evidence appears to exist.' FROM dryrun_summary
...
UNION ALL SELECT 'duplicate_product_option_count', duplicate_product_option_count, 'must be zero inside planned rows.' FROM dryrun_summary
UNION ALL SELECT 'duplicate_selfpia_product_count', duplicate_selfpia_product_count, 'must be zero inside planned rows.' FROM dryrun_summary
UNION ALL SELECT 'own_sku_multi_conflict_count', own_sku_multi_conflict_count, 'expected non-zero; own_sku repeats are reclassified evidence, not direct confirmation.' FROM dryrun_summary
UNION ALL SELECT 'semantic_warning_count', semantic_warning_count, 'semantic or quantity warning rows are not planned for later apply.' FROM dryrun_summary
...
WITH rollback_check AS (
  SELECT
    COUNT(*) AS rollback_after_count
  FROM product_code.code_alias AS ca
  WHERE ca.source_project_ref = 'smartstore_auto_match_dryrun_v1'
)
SELECT
  'rollback_after_count'::text AS dryrun_item,
  rollback_after_count AS row_count,
  'must be zero after rollback-only dryrun.'::text AS note
FROM rollback_check
UNION ALL
SELECT
  'rollback_verdict',
  CASE WHEN rollback_after_count = 0 THEN 1 ELSE 0 END,
  CASE WHEN rollback_after_count = 0 THEN 'PASS' ELSE 'FAIL' END
FROM rollback_check;
