/*
  MakeShop unique evidence candidate dryrun.

  Purpose:
  - Split the 1,247 MakeShop medium candidates into unique evidence, duplicate
    evidence, and evidence-missing buckets.
  - Dryrun only the 291 candidates whose own_sku joins to one existing MakeShop
    seller_product_code + channel_sku_code pair.
  - Do not apply confirmed aliases. This file is a BEGIN READ ONLY + ROLLBACK
    validation workflow.

  Safety:
  - SELECT-only.
  - Read-only summary/sample output.
  - No apply SQL and no persistent writes.
  - No file output.
  - No import/export.
  - No stage relation.
...
summary_rows AS (
  SELECT 'candidate_total'::text AS summary_type, COUNT(*)::bigint AS row_count, 'Original MakeShop medium candidate baseline.'::text AS note FROM candidate_rows
  ...
  UNION ALL SELECT 'insert_or_update_planned_count', COUNT(*)::bigint, 'clean planned rows after excluding existing/manual, duplicate code pairs, selfpia splits, and semantic warnings; this SQL does not write them.' FROM clean_dryrun_plan
  ...
  UNION ALL SELECT 'rollback_verdict', CASE WHEN 0 = 0 THEN 1 ELSE 0 END::bigint, 'PASS when row_count=1; this file ends with ROLLBACK.'
...
SELECT
  ...
  CASE summary_type
    ...
    WHEN 'insert_or_update_planned_count' THEN 8
    ...
    WHEN 'rollback_after_count' THEN 14
    WHEN 'rollback_verdict' THEN 15
    WHEN 'overall_verdict' THEN 16
    ELSE 99
  END AS summary_sort,
  99 AS bucket_sort,
...
SELECT
  ...
  CASE sample_bucket
    ...
    WHEN 'duplicate_evidence_excluded_sample' THEN 5
    WHEN 'duplicate_code_pair_excluded_sample' THEN 6
    WHEN 'evidence_missing_excluded_sample' THEN 7
    WHEN 'risk_keyword_sample' THEN 8
    WHEN 'same_product_family_sample' THEN 9
    WHEN 'own_sku_repeat_sample' THEN 10
    WHEN 'random_sample' THEN 11
    WHEN 'risk_edge_sample' THEN 12
    ELSE 99
  END AS bucket_sort,
